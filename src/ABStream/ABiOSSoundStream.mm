...
bool ABiOSSoundStream::setup(const ofSoundStreamSettings & settings) {
    ...
    if(settings.numInputChannels > 0) {
        soundInputStream = [[LinkSoundInputStream alloc] initWithNumOfChannels:settings.numInputChannels
        ...
        ((LinkSoundInputStream *)soundInputStream).delegate = delegate;
        [(LinkSoundInputStream *)soundInputStream start];
    }
    
    if(settings.numOutputChannels > 0) {
        soundOutputStream = [[LinkSoundOutputStream alloc] initWithNumOfChannels:settings.numOutputChannels
                                                              withSampleRate:settings.sampleRate
                                                              withBufferSize:settings.bufferSize];
        ofxiOSSoundStreamDelegate * delegate = [[ofxiOSSoundStreamDelegate alloc] initWithSoundOutputFn:settings.outCallback];
        ((LinkSoundOutputStream *)soundOutputStream).delegate = delegate;
        [(LinkSoundOutputStream *)soundOutputStream start];
    }
...
void ABiOSSoundStream::stop(){
    ...
    if(soundOutputStream != NULL) {
        [(LinkSoundOutputStream *)soundOutputStream stop];
    }
}
...
void ABiOSSoundStream::close(){
    ...
    if(soundOutputStream != NULL) {
        [((LinkSoundOutputStream *)soundOutputStream).delegate release];
        [(LinkSoundOutputStream *)soundOutputStream setDelegate:nil];
        [(LinkSoundOutputStream *)soundOutputStream stop];
        [(LinkSoundOutputStream *)soundOutputStream release];
        soundOutputStream = NULL;
    }
...
//------------------------------------------------------------------------------
LinkSoundInputStream * ABiOSSoundStream::getSoundInputStream(){
    return soundInputStream;
}

//------------------------------------------------------------------------------
LinkSoundOutputStream * ABiOSSoundStream::getSoundOutStream(){
